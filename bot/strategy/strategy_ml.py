...
def normalize_kline_df(df: pd.DataFrame) -> DataFrame | None:
...
if __name__ == "__main__":
    ...
    portfolio.account_balance = INITIAL_CAPITAL
    ...
    strategies = {item["coin"]: DualMLStrategy(portfolio, execution, item["symbol"], item["coin"]) for item in
                  TARGET_COINS}
    ...
    positions = {c: {"qty": 0.0, "entry_price": 0.0, "entry_bar": 0, "sl_pct": 0.0, "tp_pct": 0.0, "invested_cash": 0.0,
                     "entry_score": 0.0} for c in strategies}
    ...
    for i in range(start_idx, len(base_df)):
        ...
        if last_train_time is None or (curr_time - last_train_time).total_seconds() >= 86400:
            ...
            with ThreadPoolExecutor(max_workers=4) as ex:
                ...
                results = list(
                    ex.map(lambda c: strategies[c].train_models_from_df(sim_data[c].iloc[i - train_window_bars:i]),
                           coins))
        ...
        for coin, strat in strategies.items():
            ...
            if pos["qty"] > 0:
                ...
                if exit_price:
                    ...
                    print(
                        f"[{curr_time}] 卖出 [{coin}] {reason} | 价: {smart_price(exit_price)} | 赚: ${pnl:.2f} | 余额: ${portfolio.account_balance:.2f}")
                    positions[coin] = {"qty": 0.0, "entry_price": 0.0, "entry_bar": 0, "sl_pct": 0.0, "tp_pct": 0.0,
                                       "invested_cash": 0.0, "entry_score": 0.0}
        ...
        if active_cnt < MAX_POSITIONS and predictions:
            ...
            current_gross_exposure = sum(p["qty"] * float(sim_data[c].iloc[i]["open"]) for c, p in positions.items() if
                                         p["qty"] > 0 and not pd.isna(sim_data[c].iloc[i]["open"]))
            ...
            for target in predictions:
                ...
                print(
                    f"[{curr_time}] 🟢 买入 [{coin}] ({g}) | Edge: {target['expected_edge']:.4f} (Z:{target['score']:.2f}) | 投入: ${actual_cost:.2f}")
    ...
    for coin, pos in positions.items():
        if pos["qty"] > 0:
            ...
            trades_count += 1
            print(
                f"[END] 强制平仓 [{coin}] | 价: {smart_price(final_exit_price)} | 赚: ${pnl:.2f} | 余额: ${portfolio.account_balance:.2f}")
